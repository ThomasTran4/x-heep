...
// static boolean  gamekeydown[NUMKEYS]; // NRFD-TODO?
...
boolean gamekeydown(int key)
{
    //NRFD-TODO: gamekeydown
    int word_idx = key >> 5;
    uint32_t word_mask = 1<<(key&0x1F);
    uint32_t word = gamekeydown_bitmap[word_idx];
    return (word & word_mask) != 0;
}
void gamekeydown_set(int key, boolean val)
{
    int word_idx = key >> 5;
    uint32_t word_mask = 1<<(key&0x1F);
    if (val) {
...
void G_BuildTiccmd (ticcmd_t* cmd, int maketic)
{
    ...
    // strafe = strafe || mousebuttons[mousebstrafe] ;
    strafe = strafe || joybuttons[joybstrafe];
    ...
    speed = speed || joybspeed >= MAX_JOY_BUTTONS;
    speed = speed || joybuttons[joybspeed];
    ...
    if (joyymove != 0) {
        forward = -joyymove*forwardmove[speed]/50;
    }
